...
> **Note:** the code-level table initially carried placeholder numbers; actual
> PR numbers are below. The living backlog — with implemented items cleared —
> is [`ANALYSIS.md`](ANALYSIS.md). This `glm.md` is the point-in-time review.

### Plan-level (`PLAN.md`) — branches off `main`, applied cleanly
| PR | Title | PLAN sections |
|---|---|---|
| [#11](https://github.com/L-K-M/Blipbird/pull/11) | Architecture hardening: Clock, CircuitBreaker, Heartbeat, CrashRouter, rate-limit buckets | §4.2, §6, §9.1, §17 |
| [#12](https://github.com/L-K-M/Blipbird/pull/12) | Accessibility & jank budgets: designed in from M0, not audited at M4 | §9.4, §14, §18 |
| [#13](https://github.com/L-K-M/Blipbird/pull/13) | Design-token system + premium-aesthetic refinements | §9.1, §9.2, §10 |
| [#14](https://github.com/L-K-M/Blipbird/pull/14) | Delighters: Pickup Mode, route-diagram hero, sunrise alarm, ribbon scrubber, contrail, reverse-geocode | §9.2, §9.4, §11, §12, §14 |
| [#15](https://github.com/L-K-M/Blipbird/pull/15) | Harden flight identity resolution: separators, unknown-prefix, renumbering detection | §5, §9.3 |

### Code-level (real `app/` fixes — branches off `main`, verified against source)
| PR | Title | Files | Driven by |
|---|---|---|---|
| [#30](https://github.com/L-K-M/Blipbird/pull/30) | Countdown heartbeat (frozen-countdown fix) + detail-VM init order | `FlightListViewModel.kt`, `FlightDetailViewModel.kt` | 1.1, 1.12 |
| [#31](https://github.com/L-K-M/Blipbird/pull/31) | METAR decoder correctness (+RA/ceiling/MPS/multi-phenomenon) | `MetarDecoder.kt` (+test) | 1.5, 1.6, 1.7 |
| [#32](https://github.com/L-K-M/Blipbird/pull/32) | Notification/alarm correctness: ID scheme, boarding-after-airborne, stale grant label (1.2 already fixed on `main`) | `NotificationEmitter.kt`, `ReminderScheduler.kt`, `SettingsScreen.kt` | 1.3, 1.4, 5.7 |
| [#33](https://github.com/L-K-M/Blipbird/pull/33) | Network coroutine hygiene (rethrow CancellationException) + null-seenPos | `PositionProvider.kt`, `StatusProviders.kt`, `WeatherRepository.kt` | 1.8, 1.9 |
| [#34](https://github.com/L-K-M/Blipbird/pull/34) | Map geometry: near-antipodal guard, antimeridian vertex, frozen night-polygon key | `GreatCircle.kt`, `MapLibreRouteMap.kt` | 1.13, 1.14, 1.15 |
| [#35](https://github.com/L-K-M/Blipbird/pull/35) | Visual polish: ribbon perf + theme-awareness (roles, civil band, drop emoji), StatusWord contrast, progress-bar tween+Path+dead code | `Theme.kt`, `Common.kt`, `Ribbon.kt`, `FlightProgressBar.kt` | 1.17, 2.1, 2.4(progress), 3.3–3.6, 3.9 |
| [#36](https://github.com/L-K-M/Blipbird/pull/36) | Repo hygiene: CONTRIBUTING, GLOSSARY, SECURITY, ADR template, .editorconfig, .gitattributes, Gradle pin | new files + `PLAN.md` §3 | 7.1, 7.3 |

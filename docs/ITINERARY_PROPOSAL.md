...
| List sort | Two-tier global ordering: active flights by next event, landed/arrived flights sunk below | It would destroy user-authored leg order inside a group |
...
Convert to airport-local zones only for display.
...
3 -> 4 identity migration), bump `OpsDatabase` from 4 to 5 and add state,
